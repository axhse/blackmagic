...
let parseTokens text =
    ...
    let rec parse tokens restSymbols lastWord isComment isStrLiteral = 
        ...
        | ('\n')::rest when isStrLiteral -> parse (tokens @ [StringLiteral lastWord; Token.SyntaxError StringLiteralWithNoEnd; Token.Spacing '\n']) rest "" false false
        // A new line.
        ...
        // A spacing symbol.
        | first::rest when isSpacing first && isNotText -> parse ((withWord tokens lastWord) @ [Token.Spacing first]) rest "" false false
        // Any other case.
...
let parseSyntax tokens stdFunctionNames =
    ...
    let isTokenError error = error = StringLiteralWithNoEnd || error = NotSeparatedStringLiteral
    ...
    let withTokenError result restTokens error =
        match (Token.SyntaxError error)::restTokens with
            ...
            | (Token.SyntaxError error)::(Token.StringLiteral next)::rest when isTokenError error -> result @ [Literal (Literal.String next); SyntaxError StringLiteralWithNoEnd; toUnparsedCode rest]
            // Impossible situation.
            | _ -> result @ [SyntaxError error; toUnparsedCode restTokens]

    let withUnexpectedSymbol result restTokens symbol =
        result @ [SpecialSymbol symbol; SyntaxError SyntaxError.UnexcpectedSpecialSymbol; toUnparsedCode restTokens]

    let withUnexpectedLiteral result restTokens literal =
        result @ [Literal (Literal.String literal); SyntaxError SyntaxError.UnexcpectedLiteral; toUnparsedCode restTokens]
    ...
    let errorWithRest error restTokens = [SyntaxError error; toUnparsedCode restTokens]

    let withError lexeme error restTokens = [lexeme; SyntaxError error; toUnparsedCode restTokens]
    ...
    let rec parseNextDeclaration result restTokens knownFunctions =
        match restTokens with
        | [] -> result
        | (Token.SpecialSymbol symbol)::rest -> result @ withError (SpecialSymbol symbol) SyntaxError.InvalidDeclarationName rest
        | (Token.Spacing symbol)::rest -> parseNextDeclaration (result @ [Spacing symbol]) rest knownFunctions
        | (Token.Word word)::rest when isComment word -> parseNextDeclaration (result @ [Comment word]) rest knownFunctions
        | (Token.Word word)::rest -> result @ parseDeclaration rest knownFunctions word
        | (Token.StringLiteral literal)::rest -> result @ withError (Literal (Literal.String literal)) SyntaxError.InvalidDeclarationName rest
        | (Token.SyntaxError error)::rest -> withTokenError result rest error
    and parseDeclaration restTokens knownFunctions name =
        ...
        | _ when List.contains name knownFunctions -> withError (InvalidName name) SyntaxError.BusyFunctionName restTokens
    ...
    and parseFunctionTitle result restTokens knownFunctions knownVariables =
        ...
        | (Token.SpecialSymbol ':')::rest -> result @ [SpecialSymbol ':'] @ parseFunctionBody [] rest knownFunctions knownVariables
        | (Token.Spacing symbol)::rest -> parseFunctionTitle (result @ [Spacing symbol]) rest knownFunctions knownVariables
    ...
    and parseFunctionParam restTokens knownFunctions knownVariables name =
        ...
        | _ when List.contains name knownFunctions -> withError (InvalidName name) SyntaxError.BusyFunctionName restTokens
        | _ when List.contains name knownVariables -> withError (InvalidName name) SyntaxError.BusyFunctionParam restTokens
        | _ -> (FunctionParam name)::parseFunctionTitle [] restTokens knownFunctions (knownVariables @ [name])
    and parseFunctionBody result restTokens knownFunctions knownVariables =
        match restTokens with
            ...
            | (Token.SpecialSymbol '=')::rest -> 
                let expressionResult, expressionRest = parseExpression [] rest knownFunctions knownVariables
                if hasSyntaxError expressionResult
                    then result @ [SpecialSymbol '='] @ expressionResult
                else result @ [SpecialSymbol '='] @ expressionResult @ parseNextDeclaration [] expressionRest knownFunctions
            | (Token.Spacing symbol)::rest -> parseFunctionBody (result @ [Spacing symbol]) rest knownFunctions knownVariables
            | (Token.Word word)::rest when isComment word -> parseFunctionBody (result @ [Comment word]) rest knownFunctions knownVariables
    ...
    and parseBlockStartName restTokens knownFunctions knownVariables name =
        match name with
        | _ when not (isValidName name) -> withError (InvalidName name) SyntaxError.InvalidName restTokens
        | _ when List.contains name knownFunctions -> withError (InvalidName name) SyntaxError.AssigmentWasExpected restTokens
        | _ -> (Lexeme.Assignment name)::parseAssignment [] restTokens knownFunctions (knownVariables @ [name])
    and parseAssignment result restTokens knownFunctions knownVariables = 
        match restTokens with
            ...
            | (Token.SpecialSymbol '=')::rest -> 
                ...
                if hasSyntaxError expressionResult
                    then result @ [SpecialSymbol '='] @ expressionResult
                else parseFunctionBody (result @ [SpecialSymbol '='] @ expressionResult) expressionRest knownFunctions knownVariables
            | (Token.Spacing symbol)::rest -> parseAssignment (result @ [Spacing symbol]) rest knownFunctions knownVariables
            | (Token.Word word)::rest when isComment word -> parseFunctionBody (result @ [Comment word]) rest knownFunctions knownVariables
    ...
    and parseExpression result restTokens knownFunctions knownVariables = 
        match restTokens with
            ...
            | (Token.SpecialSymbol '(')::rest ->
                let containerResult, containerRest = parseContainerContent rest knownFunctions knownVariables
                if hasSyntaxError containerResult
                    then result @ [SpecialSymbol '('] @ containerResult, containerRest
                else parseExpression (result @ [SpecialSymbol '('] @ containerResult) containerRest knownFunctions knownVariables
            | (Token.SpecialSymbol ')')::rest -> result, Token.SpecialSymbol ')'::rest
            | (Token.SpecialSymbol ';')::rest ->
                if isExpressionMeaningful result
                then result @ [SpecialSymbol ';'], rest
                else result @ withError (SpecialSymbol ';') SyntaxError.EmptyExpression rest, []
            | (Token.Spacing symbol)::rest -> parseExpression (result @ [Spacing symbol]) rest knownFunctions knownVariables
            | (Token.Word word)::rest when isComment word -> parseExpression (result @ [Comment word]) rest knownFunctions knownVariables
            | (Token.Word word)::rest ->
                ...
                else parseExpression (result @ wordResult) rest knownFunctions knownVariables
            | (Token.StringLiteral literal)::rest -> parseExpression (result @ [Literal (Literal.String literal)]) rest knownFunctions knownVariables
            | _ -> (withUnexpectedRest result restTokens, [])
    ...
    and parseContainerContent restTokens knownFunctions knownVariables =
        let expressionResult, expressionRest = parseExpression [] restTokens knownFunctions knownVariables
        if not (isExpressionMeaningful expressionResult)
            then expressionResult @ errorWithRest SyntaxError.EmptyParethesisBlock expressionRest, []
        else
            if hasSyntaxError expressionResult
                then expressionResult, expressionRest
            else 
                match expressionRest with
                    ...
                    | (Token.SpecialSymbol ';')::rest -> expressionResult @ withError (SpecialSymbol ';') SyntaxError.MissingParenthesis rest, []
