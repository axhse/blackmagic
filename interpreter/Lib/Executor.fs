...
let findDeclaration name code =List.find (fun (decl: Declaration) -> decl.Name = name) code
...
let updateVariable (variables: Variable list) (name: string) (value: Value) =
    if hasVariable variables name
        then List.map (fun x -> if x.Name = name then {Name = name; Value = value} else x) variables
    else variables @ [{Name = name; Value = value}]
...
let rec apply (value: Value) arg code invocationDepth =
    ...
    | Value.Function func ->
        ...
        if paramCount <= List.length func.Args
            then failwith "Unexpected application."
        else
...
and evaluateExpression expression variables invocationDepth code =
    ...
    | Access name ->
        evaluateAccess variables invocationDepth code name
...
and evaluateAccess variables invocationDepth code name =
    if hasVariable variables name
        then Value (getVariableValue variables name)
        else createFunction name invocationDepth code
and evaluateComposition accumulator restExpressions variables invocationDepth code =
    match restExpressions with
    | [] -> Value accumulator
    | expression::rest ->
        let expressionResult: ExecutionResult = evaluateExpression expression variables invocationDepth code
        match expressionResult with
        | RuntimeError message -> RuntimeError message
        | Value arg ->
...
and createFunction name invocationDepth code =
    let paramCount =
        if isBuiltInFunction name
            then getBuiltInFunctionParamCount name
        else
            let declaration = findDeclaration name code
            List.length declaration.ParamNames
    let func = { Name = name; Args = []; ParamCount = paramCount }
    if paramCount > 0 then Value (Value.Function func) else evaluateFunction func code invocationDepth
...
let execute (code: Declaration list) (functionName: string) (args: Value list): ExecutionResult =
    ...
    | _ when isBuiltInFunction functionName ->
        ...
        if paramCount < argCount
            then RuntimeError (sprintf "Function `%s` has only %d parameters. Got %d arguments." functionName paramCount argCount)
            else Value (invokeBuiltIn functionName args)
    | _ when List.exists (fun (decl: Declaration) -> decl.Name = functionName) code ->
        ...
        if paramCount < argCount
            then RuntimeError (sprintf "Function `%s` has only %d parameters. Got %d arguments." functionName paramCount argCount)
            else executeDeclaredFunction code declaration args
